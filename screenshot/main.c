/*
 * gcc main.c -pthread -lX11 -lXext -m64 -mavx2 -o fltest -O3 && sudo ./fltest 1
 * Sample program provided by FrescoLogic modified to take a screenshot and
 * write it to framebuffer of the FL2000. 1920*1080 24bpp only, you can change
 * the height-width if you want.

 * Takes the screenshot of the left monitor, sends it to FL2000. This is a bad
 * idea, but it works. (Cursor is invisible, there is a lot of performance you can
 * gain by cleaning up unnecessary stuff)
 */
#include <X11/X.h>
#include <X11/Xlib.h>
#include <X11/Xutil.h>
...
//#define	COMPRESS_SIZE_LIMIT			1666666		// for
// xHC that can do 100MB/s, the driver compress each frame to size no larger
// than (100*1000*1000/60) bytes
...
void fl2000_monitor_ratio_to_dimension(uint8_t x, uint8_t aspect_ratio,
...
void parse_edid(void) {
  ...
  for (i = 38; i < 53; i += 2) {
    ...
    fl2000_monitor_ratio_to_dimension(x, ratio, &width, &height);
...
void write_to_buffer(uint8_t *frame_buffer, uint32_t width, uint32_t height,
                     Display *display, XImage *ximg) {
  XColor c;
  uint8_t *pixel;
  XShmGetImage(display, root, ximg, 0, 0, 0x00ffffff);
  if (ximg->data != NULL) {
    // set framebuffer's pixels to screenshot's pixels (32bpp -> 24bpp)
    for (uint32_t y = 0; y < height; y++) {
      for (uint32_t x = 0; x < width; x++) {
        pixel = frame_buffer + y * (width * 3) + x * 3;
        c.pixel = XGetPixel(ximg, x, y);
        pixel[0] = (c.pixel & ximg->blue_mask);
        pixel[1] = (c.pixel & ximg->green_mask) >> 8;
        pixel[2] = (c.pixel & ximg->red_mask) >> 16;
      }
    }
  }
}

void init_frame_by_test_pattern(uint8_t *frame_buffer, uint32_t width,
                                uint32_t height) {
  uint32_t y;
  uint32_t x;
  int color = 0;
 ...
 */
bool init_frame_by_bmp_file(uint8_t *frame_buffer, uint32_t width,
  ...
  snprintf(file_name, sizeof(file_name), "test_%d_%d_%d.bmp", width, height,
           index);
...
int _alloc_frame(int fd, struct test_alloc *phy_alloc, uint32_t size) {
  ...
  if (ret < 0)
    fprintf(stderr, "%s: IOCTL_FL2000_TEST_ALLOC_SURFACE failed %d\n", __func__,
            ret);
...
void _release_frame(int fd, struct test_alloc *phy_alloc) {
  ...
  if (ret < 0)
    fprintf(stderr, "%s: IOCTL_FL2000_TEST_RELEASE_SURFACE failed %d\n",
            __func__, ret);
}
...
void test_display(int fd, uint32_t width, uint32_t height, uint32_t index) {
  ...
  while (true) {
    write_to_buffer(frame_buffer, width, height, display, ximg);
...
void test_display_all(int fd) { test_display(fd, 1920, 1080, 0); }
...
void test_display_on_resolution(int fd, uint32_t width, uint32_t height) {
  ...
  ret_val = pthread_create(&monitor_thread, NULL, monitor_connection_worker,
                           (void *)((intptr_t)fd));
  if (ret_val != 0) {
...
void main(int argc, char *argv[]) {
  ...
  usage:
    ...
    fprintf(stderr,
            ...
            argv[0]);
    fprintf(stderr,
            "eg2: to test without SURFACE_TYPE_VIRTUAL_FRAGMENTED_PERSISTENT, "
            "type\n"
            "%s 1\n",
            argv[0]);
    fprintf(stderr,
            "eg3: to test 1920x1080 with "
            "SURFACE_TYPE_VIRTUAL_FRAGMENTED_PERSISTENT, type\n"
            "%s 1 1920 1080\n",
            argv[0]);
    goto exit;
  }
